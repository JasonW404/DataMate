...
@project_router.post("/sync", response_model=SyncDatasetResponse)
async def sync_dataset_content(
    ...
    try:
        ...
        logger.info(f"同步数据集内容请求: mapping_id={request.mapping_id}")
        ...
        if not mapping:
            raise HTTPException(
                status_code=404,
                detail=f"未找到映射: {request.mapping_id}"
            )
        
        # 执行同步（使用映射中的源数据集UUID）
        result = await sync_service.sync_dataset_files(request.mapping_id, request.batch_size)
        
        logger.info(f"同步完成: {result.synced_files}/{result.total_files} 文件")
        ...
        logger.error(f"数据集信息获取失败: {e}")
        raise HTTPException(status_code=404, detail=str(e))
    except DatasetMappingNotFoundError as e:
        logger.error(f"映射关系未找到: {e}")
        raise HTTPException(status_code=404, detail=str(e))
    except Exception as e:
        logger.error(f"同步数据集内容时发生错误: {e}")
