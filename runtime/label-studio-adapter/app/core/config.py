...
class Settings(BaseSettings):
    ...
    # JWT Token (refresh token) - 用于 JWT 认证方式
    label_studio_api_key: Optional[str] = None
    # Legacy Token - 用于 Token 认证方式（推荐，默认使用）
    label_studio_user_token: Optional[str] = None
    dm_service_base_url: str = "http://data-engine"
    
    # 文件路径配置
    dm_file_path_prefix: str = "/uploads/"  # DM存储文件夹前缀
    ...
    # 日志配置
    log_level: str = "INFO"
    # Label Studio tasks list page size (used when fetching tasks via SDK)
